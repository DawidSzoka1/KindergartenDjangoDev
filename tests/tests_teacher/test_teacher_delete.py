...
def test_teacher_delete_access_1(client_conf, client_parent, client_director):
    ...
    response = client_conf.get(f"/employee/delete/{teacher.id}/")
    assert response.status_code == 302
    response_parent = client_parent['client'].get(f"/employee/delete/{teacher.id}/")
    assert response_parent.status_code == 403
    response = client_director['client'].get(f"/employee/delete/{teacher.id}/")
...
def test_teacher_delete_access_2(client_director, client_parent):
    ...
    response_teacher = client_parent['client'].post(f"/employee/delete/{teacher.id}/")
    assert response_teacher.status_code == 403
    response = client_director['client'].post(f"/employee/delete/{teacher.id}/")
    assert response.status_code == 302
    assert not Employee.objects.filter(id=teacher.id).first()
    assert not User.objects.filter(id=teacher.user.id).first()
